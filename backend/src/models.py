...
class KpiTemplate(db.Entity):
    id = PrimaryKey(int, auto=True)
    name = Required(str)
    fields_json = Optional(str)
    is_active = Required(bool, default=False)
    created_at = Required(datetime, default=datetime.utcnow)
    reports = Set("KpiReport")


class KpiReport(db.Entity):
    id = PrimaryKey(int, auto=True)
    user = Required(User)
    template = Required(KpiTemplate)
    week_start = Required(date)
    answers_json = Optional(str)
    submitted_at = Required(datetime, default=datetime.utcnow)
