#!/usr/bin/env python3
"""
Migración: agrega `requested_next_phase` a la tabla ActivationTask (Pony ORM / Neon).

Lee `backend/.env` por ruta absoluta (funciona desde cualquier carpeta).

  cd backend && python ../scripts/add_activation_task_requested_next_phase.py

Variables en backend/.env (como en `src/db.py`):
  - DATABASE_URL (recomendado Neon: postgresql://...?sslmode=require), o
  - DB_PROVIDER=postgres y DB_USER, DB_PASS, DB_HOST, DB_NAME
  - Opcional sin URL: DB_SSLMODE=require
"""
...
COLUMN = "requested_next_phase"


def _load_env_file(path: Path) -> dict[str, str]:
    """Parseo mínimo de .env; soporta líneas KEY=valor y comillas."""
    out: dict[str, str] = {}
    if not path.is_file():
        return out
...
def _find_activation_tables(cur) -> list[tuple[str, str]]:
    """Resuelve nombre real de tabla (Pony/Postgres puede variar mayúsculas)."""
    cur.execute(
        """
        ...
        WHERE table_type = 'BASE TABLE'
          ...
          AND (
            lower(table_name) = 'activationtask'
            OR table_name = 'ActivationTask'
          )
    ...
    cur.execute(
        ...
        WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
          AND (
            lower(tablename) = 'activationtask'
            OR tablename = 'ActivationTask'
          )
...
def migrate_postgres(env: dict[str, str], cfg) -> None:
    ...
    rows = _find_activation_tables(cur)
    if not rows:
        print(
            "No se encontró la tabla de ActivationTask.",
            file=sys.stderr,
        )
        sys.exit(1)
...
def migrate_sqlite(env: dict[str, str], cfg) -> None:
    ...
    cur.execute(
        ...
        "(lower(name) = 'activationtask' OR name = 'ActivationTask')"
    )
    tables = [r[0] for r in cur.fetchall()]
    if not tables:
        print("No se encontró tabla activationtask en SQLite.", file=sys.stderr)
        sys.exit(1)
