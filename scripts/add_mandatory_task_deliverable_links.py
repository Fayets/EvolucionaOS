#!/usr/bin/env python3
"""
Migración: agrega `deliverable_links` (TEXT, JSON array de URLs) a MandatoryTask.

  cd backend && python ../scripts/add_mandatory_task_deliverable_links.py

Misma configuración que otros scripts (`backend/.env`, DATABASE_URL o DB_*).
"""
...
def _find_mandatory_task_tables(cur) -> list[tuple[str, str]]:
    cur.execute(
        """
        ...
        WHERE table_type = 'BASE TABLE'
          ...
          AND (
            lower(table_name) = 'mandatorytask'
            OR table_name = 'MandatoryTask'
          )
    ...
    cur.execute(
        ...
        WHERE schemaname NOT IN ('pg_catalog', 'information_schema')
          AND (
            lower(tablename) = 'mandatorytask'
            OR tablename = 'MandatoryTask'
          )
...
def migrate_postgres(env: dict[str, str], cfg) -> None:
    ...
    rows = _find_mandatory_task_tables(cur)
    if not rows:
        print("No se encontró la tabla MandatoryTask.", file=sys.stderr)
        sys.exit(1)
...
def migrate_sqlite(env: dict[str, str], cfg) -> None:
    ...
    cur.execute(
        ...
        "(lower(name) = 'mandatorytask' OR name = 'MandatoryTask')"
    )
    tables = [r[0] for r in cur.fetchall()]
    if not tables:
        print("No se encontró tabla mandatorytask en SQLite.", file=sys.stderr)
        sys.exit(1)
